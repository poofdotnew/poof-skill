...
| ---------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `poof build -m "..." [--mode MODE] [--public] [--stdin] [--file img.png] [--network NET]`   | Create a project, wait for AI to finish. Returns project ID and URLs. Mode: `full` (default), `policy`, `ui,policy`, `backend,policy`. Bare values `ui` and `backend` are also accepted (policy is auto-included). Attach reference images (PNG, JPEG, GIF, WebP; ≤3.4MB each) with `--file` — the CLI pre-uploads each image to Poof's global storage and embeds the URLs in the initial firstMessage so the AI sees text + images together. |
| `poof iterate -p <id> -m "..." [--stdin] [--file img.png]` | Send a chat message, wait for AI to finish, show test results. Attach images with `--file`. JSON output includes both `summary` and `freshSummary`. `freshSummary` is the per-file collapse of the rows in the after-snapshot's page-of-100 that weren't in the before-snapshot's; `summary` is the per-file collapse of the after-snapshot's page itself. Text output uses `freshSummary` and says "no tests ran during this turn" when the collapsed-fresh view is empty. See [Verify & Test Diagnostics](verify-diagnostics.md) for what the bounded-page diff means. For strict pass/fail gates use `poof verify` instead. |
| `poof verify -p <id> [--ui-tests auto\|true\|false] [--skip-url-probe] [-m "..."]` | Send canonical verification prompt, wait for AI, validate fresh test results. "Fresh" here means rows visible in the after-snapshot's bounded result page (single page of 100) but not the before-snapshot's, then collapsed per file (`source\|fileName`) before pass/fail evaluation — see [Verify & Test Diagnostics](verify-diagnostics.md). Auto-detects backend-only/static-deploy projects and skips UI tests. Exits non-zero if no fresh results or any failure. Use `--ui-tests=false` to force lifecycle-only. Use `--ui-tests=true` only for Poof-generated UIs or for static deploys where source-authored `lifecycle-actions/ui-test-*.json` files were uploaded first; pair it with `-m` to say "run existing tests, do not generate from dist." |
...
| `poof project create -m "..." [--mode MODE] [--public] [--stdin] [--network NET]`                                          | Create a project (AI starts building). Does NOT wait — use `poof build` instead. Mode: `full` (default), `policy`, `ui,policy`, `backend,policy`. Bare values `ui` and `backend` are also accepted (policy is auto-included). |
...
| `poof task test-results -p <id> [--limit N] [--offset N] [--history]` | Structured policy + UI test results — pass/fail, errors, counts. Default limit: 100. Results are collapsed to latest run per test file by default. Pass `--history` to bypass the CLI's per-file collapse and show one row per `(source, fileName, testName)` as returned by the server. The server's response is already deduped; `--history` does not surface raw run history. See [Verify & Test Diagnostics](verify-diagnostics.md) for ghost-row caveats. |
...
| `poof deploy backend -p <id> --archive backend-worker.tar.gz [--title T] [--description D] [--dry-run]`            | Deploy a pre-built PartyServer backend Worker artifact to draft. Archive must be Wrangler-bundled output with `poof-backend-artifact.json`; preview/production deploys preserve the active artifact.                               |
...
| Command                                                                             | Description                                                                                                                                                                                                                                      |
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `poof verify -p <id> [--ui-tests auto\|true\|false] [--skip-url-probe] [-m "..."]` | Canonical post-build test gate. Snapshots existing tests, sends verification prompt, waits for AI, then counts "fresh" results — defined as rows in the after-snapshot's bounded result page (single page of 100) but not the before-snapshot's, then collapsed per file (`source\|fileName`). Exits non-zero if no fresh results or any failure in the collapsed view. See [Verify & Test Diagnostics](verify-diagnostics.md) for full caveats. Auto-detects backend-only/static-deploy projects and skips UI tests unless forced. For static deploys, force UI tests only to run existing source-authored `ui-test-*.json` files. |
...
| `--network <net>`     | Select the database network (used with `build` and `project create`). Values: `realtime` (alias for `realtime_offchain`), `realtime_devnet`, `realtime_mainnet`. Realtime uses Cloudflare Durable Objects for ultra-low-latency reads/writes and instant subscriptions. |
...
| `poof deploy static`     | `{ target: "static", projectId, taskId, bundleUrl, urls: {...} }`                                                                                                                                                                      |
| `poof deploy backend`    | `{ target: "backend", projectId, taskId, backendUrl, urls: {...} }`                                                                                                                                                                    |
| `poof iterate`           | `{ results: [...], summary: {total, passed, failed, errors, running}, freshResults: [...], freshSummary: {total, passed, failed, errors, running}, hasMore }` — `freshResults`/`freshSummary` are derived from a bounded before/after diff of the result-page IDs around this turn (single page of 100 in each snapshot), then collapsed per file (`source\|fileName`) — a row appears in `freshResults` iff it's in the after-snapshot's page but not the before-snapshot's *and* is the latest for its file in that fresh set. `results`/`summary` are the CLI's per-file collapse over the after-snapshot's page itself. The server fetches a bounded raw window before dedupe and slicing, so neither side is guaranteed to represent all history. See [Verify & Test Diagnostics](verify-diagnostics.md) for the full caveat. |
